...
| File | What it is |
|---|---|
| `snaked-itch.zip` | The game for itch's upload box: index.html, manifest, sw.js, icons, og.png, favicon (rebuilt 19 Sep from commit a47ac9c: Esc-only pause, d-pad below the board, race with Breakin's screens, You win/You lost, 30s two-player rematch vote, leaving ends the match, 5s countdown before leaving a finished match). If index.html changes before posting, rebuild it: `git -C <snaked folder> archive --format=zip -o press/snaked-itch.zip HEAD index.html manifest.json sw.js favicon.svg icon-192.png icon-512.png og.png`. |
