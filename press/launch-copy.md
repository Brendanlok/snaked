...
| File | What it is |
|---|---|
| `snaked-itch.zip` | The game for itch's upload box: index.html, manifest, sw.js, icons, og.png, favicon (rebuilt 19 Sep from commit 316c8b8: name boxes never pre-filled, Esc-only pause, d-pad below the board, race with Breakin's screens, You win/You lost, 30s two-player rematch vote, leaving ends the match, 5s countdown before leaving a finished match, both boards on the race result, race runs until both are out, times on the race result, rival apples left + time in the race HUD). If index.html changes before posting, rebuild it: `git -C <snaked folder> archive --format=zip -o press/snaked-itch.zip HEAD index.html manifest.json sw.js favicon.svg icon-192.png icon-512.png og.png`. |
| `snaked-run.gif` | 27s loop, 320x512, 2.4 MB, re-recorded 19 Sep in Breakin's style: the board only (no score tiles, no title card). One full winning run by the demo bot at real speed: 30 pieces down to 1, the board filling with stone, the speed-up at the end. |
| `itch-cover.png` | 630x500 itch cover in Breakin's layout (19 Sep): SNAKED on the grid, the snake framing it top and right, apple and stone in the corners. |
| `shot-0-menu.png` | Menu at phone width (1000x2080, 2x), re-shot 19 Sep with Play competitive; fits itch's 2160px limit. |
